...
{{--            <button class="blue-btn">Create</button>--}}
        ...
        <div id="filter-panel" class="filter-panel hidden">
            ...
            <div class="filter-section">
                <h3>Status</h3>
                <label><input type="checkbox" /> Slots available</label>
                <label><input type="checkbox" /> In registration</label>
            </div>
...
{{--                                </a>--}}
        ...
        <div class="pagination">
            ...
            @if ($currentPage > $pageRange + 1)
                <a href="{{ $activities->url(1) }}" class="pagination-page">1</a>
                <span class="gap">...</span>
            @endif

            @foreach (range($currentPage - $pageRange, $currentPage + $pageRange) as $page)
                @if ($page > 0 && $page <= $lastPage)
                    @if ($page == $currentPage)
                        <span class="pagination-page current">{{ $page }}</span>
                    @else
                        <a href="{{ $activities->url($page) }}" class="pagination-page">{{ $page }}</a>
                    @endif
                @endif
            @endforeach
            ...
            @if ($currentPage < $lastPage - $pageRange)
                <span class="gap">...</span>
                <a href="{{ $activities->url($lastPage) }}" class="pagination-page">{{ $lastPage }}</a>
            @endif
