...
{{--            </form>--}}
            ...
            <form id="filter-form" class="filter-panel hidden" action="{{ route('activity.participants', ['activity' => $activity->id]) }}" method="GET">
                ...
                <div class="filter-section">
                    <h3>Regis_Date</h3>
                    <input type="date" name="regis_date" placeholder="Start Date" />
                </div>
                <div class="filter-section">
                    <h3>Building</h3>
                        ...
                        <td>
                            @if($participant->student && $participant->student->residence)
                                @php
                                    $hasRoom = false;
                                @endphp
                                @foreach($participant->student->residence as $residence)
                                    @if(in_array($residence->status, ['Paid', 'Checked in', 'Registered']))
                                        <p>{{ $residence->room->name ?? 'No room available' }}</p>
                                        @php
                                            $hasRoom = true;
                                        @endphp
                                    @endif
...
{{--                                    </form>--}}
            ...
            <div class="pagination">
                ...
                @else
                    <a href="{{ request()->fullUrlWithQuery(['page' => $participants->currentPage() - 1]) }}" class="previous">Previous</a>
                @endif
                ...
                @if ($participants->hasMorePages())
                    <a href="{{ request()->fullUrlWithQuery(['page' => $participants->currentPage() + 1]) }}" class="next">Next</a>
                @else
            ...
            <script>
                function goBack() {
                    window.location.href = "{{ route('activities.show', ['id' => $activity->id]) }}";
                }
            </script>
            <p style="text-align: right;margin-top: -20px">{{ $participants->currentPage() }} / {{ $participants->lastPage() }}</p>
        ...
        <div class="extension">
            ...
            <div>
                <p style="color: var(--Close, #FF2F5C);
                    ...
                    Only admintrasion and building manager can see this page!
                </p>
            </div>
        </div>
    @endif
