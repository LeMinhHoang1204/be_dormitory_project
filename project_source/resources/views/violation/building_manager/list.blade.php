...
@section('content')
    ...
    <div class="extension">
        <h3 class="heading"> Violation List </h3>
        ...
        <table class="table">
            ...
            <tbody>
            @forelse ($violations as $indexManager => $violation)
                <tr>
                    <td>{{ $indexManager + 1 }}</td>
                    <td>{{ $violation->id }}</td>
                    <td>{{ optional($violation->creator)->name ?? 'N/A' }}</td>
                    <td>{{ optional($violation->receiver->student->user)->name ?? 'N/A' }}</td>
                    <td>
                        @if ($violation->receiver->student->latestResidence)
